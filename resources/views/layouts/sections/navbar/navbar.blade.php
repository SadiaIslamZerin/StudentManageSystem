                    ...
                    <a class="dropdown-item" href="javascript:void(0);">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1">
                                <h6 class="mb-0 small" id = "UserNameZeden">John Doe</h6>
                                <small class="text-muted" id = "UserRoleZeden">Admin</small>
                            </div>
                        </div>
                    </a>
                </li>
...
<script>
    function checkSession() {
        @if (session()->has('user_logged_in'))
            console.log('Session Value:', @json(session('user_logged_in')));
            console.log('Session Value:', @json(session('lifetime')));
        @else
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var UserName = document.getElementById('UserNameZeden');
        var UserRole = document.getElementById('UserRoleZeden');

        UserName.textContent = @json(session('userName'));
        UserRole.textContent = @json(session('userRole'));
    });
</script>
